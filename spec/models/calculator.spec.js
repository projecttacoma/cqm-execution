...
describe('Calculator', () => {
  describe('Continuous Variable Calculations', () => {
    it('can handle single episodes observed', async () => {
      ...
      const calculationResults = await Calculator.calculate(measure, patients, valueSets);
  ...
  describe('episode of care based relevance map', () => {
    it('is correct for patient with no episodes', async () => {
    ...
    xit('is correct for patient with episodes', async () => {
  ...
  describe('patient based relevance map', () => {
    it('doesnt fail calculation of measure observations', async () => {
  ...
  describe('execution engine using passed in timezone offset', () => {
    xit('is correct', async () => {
      ...
      const patients = [];
      patients.push(getJSONFixture('patients/CMS760v0/Correct_Timezone.json').qdmPatient);
      const calculationResults = await Calculator.calculate(measure, patients, valueSets, { timezoneOffset: -4, requestDocument: true });
      const result = Object.values(calculationResults[Object.keys(calculationResults)[0]])[0];
  ...
  describe('execution engine using passed in timezone offset and good effective_date', () => {
    xit('is correct', async () => {
      ...
      const patients = [];
      patients.push(getJSONFixture('patients/CMS760v0/Correct_Timezone.json').qdmPatient);
      const calculationResults = await Calculator.calculate(measure, patients, valueSets, { effectiveDate: '201201010000', timezoneOffset: -4, requestDocument: true });
      const result = Object.values(calculationResults[Object.keys(calculationResults)[0]])[0];
  ...
  describe('execution engine using passed in timezone offset and bad effective_date',  () => {
    it('is correct', async () => {
      ...
      const patients = [];
      patients.push(getJSONFixture('patients/CMS760v0/Correct_Timezone.json').qdmPatient);
      const calculationResults = await Calculator.calculate(measure, patients, valueSets, { effective_date: '201701010000', timezone_offset: -4, requestDocument: true });
      const result = Object.values(calculationResults[Object.keys(calculationResults)[0]])[0];
  ...
  it('multiple population measure correctly', async () => {
  ...
  it('multiple population and stratification measure correctly', async () => {
    ...
    const calculationResults = await Calculator.calculate(measure, patients, valueSets, options);
  ...
  xit('multiple observation measure correctly without mongoose document', async () => {
    // TODO: Find another measure to use. PrincipalDiagnosis is no longer a QDM attribute and is used in this measure logic
    ...
    const calculationResults = await Calculator.calculate(measure, patients, valueSets, { requestDocument: false });
  ...
  it('single population patient-based measure correctly', async () => {
  ...
  describe('opioid measure', () => {
    it('results have correct indentation', async () => {
  ...
  describe('results include clause results', () => {
    it('only if requested with document', async () => {
      const valueSets = getJSONFixture('cqm_measures/CMS760v0/value_sets.json');
      const measure = getJSONFixture('cqm_measures/CMS760v0/CMS760v0.json');
      const patients = [getJSONFixture('patients/CMS760v0/Correct_Timezone.json')];
      const calculationResultsNoClauses = await Calculator.calculate(measure, patients, valueSets, { includeClauseResults: false, requestDocument: true });
      const resultNoClauses = Object.values(calculationResultsNoClauses[Object.keys(calculationResultsNoClauses)[0]])[0];

      const calculationResultsWithClauses = await Calculator.calculate(measure, patients, valueSets, { includeClauseResults: true, requestDocument: true });
    ...
    it('only if requested without document', async () => {
      const valueSets = getJSONFixture('cqm_measures/CMS760v0/value_sets.json');
      const measure = getJSONFixture('cqm_measures/CMS760v0/CMS760v0.json');
      const patients = [getJSONFixture('patients/CMS760v0/Correct_Timezone.json')];
      const calculationResultsNoClauses = await Calculator.calculate(measure, patients, valueSets, { includeClauseResults: false, requestDocument: false });
      const resultNoClauses = Object.values(calculationResultsNoClauses[Object.keys(calculationResultsNoClauses)[0]])[0];

      const calculationResultsWithClauses = await Calculator.calculate(measure, patients, valueSets, { includeClauseResults: true, requestDocument: false });
      const resultWithClauses = Object.values(calculationResultsWithClauses[Object.keys(calculationResultsWithClauses)[0]])[0];
  ...
  describe('patient characteristics', () => {
    it('calculate and display correctly', async () => {
  ...
  describe('Id objects', () => {
    it('properly calculate and print', async () => {
...
START: 05/11/2012 8:00 AM\nSTOP: 05/11/2012 8:15 AM\nCODE: SNOMEDCT 428411000124104]`);
  ...
  describe('Non-Versioned Value-Sets', () => {
    xit('Calculates correctly', async () => {
      // TODO: Find another measure to use. PrincipalDiagnosis is no longer a QDM attribute and is used in this measure logic
  ...
  describe('Using cqm-models passed in', () => {
    it('Doesn\'t pollute Measure model to calculate measure with stratification', async () => {
      const valueSets = getJSONFixture('cqm_measures/CMS160v7/value_sets.json');
      const measure = getJSONFixture('cqm_measures/CMS160v7/CMS160v7.json');
      const numPass = getJSONFixture('patients/CMS160v7/PHQ9EBEDec_PerDzDxSAEDec_NUM1Pass.json');
      const patients = [];
      ...
      const options = { doPretty: true, requestDocument: true };
      const calculationResults = await Calculator.calculate(cqmMeasure, patients, valueSets, options);
      const numPassResults = calculationResults[Object.keys(calculationResults)[0]];
