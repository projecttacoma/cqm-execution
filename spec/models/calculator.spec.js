...
const getEpisodeResults = require("../support/spec_helper.js")
  .getEpisodeResults;

describe("Calculator", () => {
  describe("episode of care based relevance map", () => {
    it.skip("is correct for patient with no episodes", () => {
      ...
      const patients = [];
      patients.push(
        getJSONFixture("patients/CMS107v6/IPPFail_LOS=121Days.json")
      );
  ...
  describe("measure that uses intersect function ", () => {
    xit("is correct", () => {
      ...
      const patients = [];
      patients.push(
        getJSONFixture("patients/CMS53v7/PCIat60min_NUMERPass.json")
      );
      const calculationResults = Calculator.calculate(
        measure,
        patients,
  ...
  it.skip("multiple population measure correctly", () => {
    ...
    const expiredDenexResults =
      calculationResults[Object.keys(calculationResults)[0]];
    const passNumer2Results =
      calculationResults[Object.keys(calculationResults)[1]];
  ...
  it.skip("multiple population and stratification measure correctly", () => {
    ...
    const numPass = getJSONFixture(
      "patients/CMS160v7/PHQ9EBEDec_PerDzDxSAEDec_NUM1Pass.json"
    );
    const patients = [];
    ...
    const numPassResults =
      calculationResults[Object.keys(calculationResults)[0]];

    // Patient expiredDenexResults Population Set 1
    const pop2Strat1 = numPassResults["PopulationCriteria2"];
    const pop2Strat1StatementResults = pop2Strat1.statement_results_by_statement()
      .DepressionUtilizationofthePHQ9Tool;
    expect(
      pop2Strat1StatementResults["May through August of Measurement Period"]
        .pretty
    ).toBe("INTERVAL: 05/01/2012 12:00 AM - 09/01/2012 12:00 AM");
  });
  ...
  xit("multiple observation measure correctly without mongoose document", () => {
    ...
    const visit1EdResults =
      calculationResults[Object.keys(calculationResults)[0]];
    const visit2EdResults =
      calculationResults[Object.keys(calculationResults)[1]];
    const visit1exclResults =
      calculationResults[Object.keys(calculationResults)[2]];
    const visit2exclResults =
      calculationResults[Object.keys(calculationResults)[3]];
    ...
    expect(
      getEpisodeResults(visit1EdResults["PopulationCriteria1"].episode_results)
    ).toEqual([15]);
    // Patient visit1EdResults Population Set 1 Stratification 1
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 1"].STRAT
    ).toBe(1);
    expect(visit1EdResults["PopulationCriteria1 - Stratification 1"].IPP).toBe(
      1
    );
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 1"].MSRPOPL
    ).toBe(1);
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 1"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit1EdResults["PopulationCriteria1 - Stratification 1"]
          .episode_results
      )
    ).toEqual([15]);
    // Patient visit1EdResults Population Set 1 Stratification 2
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 2"].STRAT
    ).toBe(0);
    expect(visit1EdResults["PopulationCriteria1 - Stratification 2"].IPP).toBe(
      0
    );
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 2"].MSRPOPL
    ).toBe(0);
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 2"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit1EdResults["PopulationCriteria1 - Stratification 2"]
          .episode_results
      )
    ).toEqual([]);
    // Patient visit1EdResults Population Set 1 Stratification 3
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 3"].STRAT
    ).toBe(0);
    expect(visit1EdResults["PopulationCriteria1 - Stratification 3"].IPP).toBe(
      0
    );
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 3"].MSRPOPL
    ).toBe(0);
    expect(
      visit1EdResults["PopulationCriteria1 - Stratification 3"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit1EdResults["PopulationCriteria1 - Stratification 3"]
          .episode_results
      )
    ).toEqual([]);
    ...
    expect(
      getEpisodeResults(visit2EdResults["PopulationCriteria1"].episode_results)
    ).toEqual([15, 25]);
    // Patient visit2EdResults Population Set 1 Stratification 1
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 1"].STRAT
    ).toBe(2);
    expect(visit2EdResults["PopulationCriteria1 - Stratification 1"].IPP).toBe(
      2
    );
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 1"].MSRPOPL
    ).toBe(2);
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 1"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit2EdResults["PopulationCriteria1 - Stratification 1"]
          .episode_results
      )
    ).toEqual([15, 25]);
    // Patient visit2EdResults Population Set 1 Stratification 2
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 2"].STRAT
    ).toBe(0);
    expect(visit2EdResults["PopulationCriteria1 - Stratification 2"].IPP).toBe(
      0
    );
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 2"].MSRPOPL
    ).toBe(0);
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 2"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit2EdResults["PopulationCriteria1 - Stratification 2"]
          .episode_results
      )
    ).toEqual([]);
    // Patient visit2EdResults Population Set 1 Stratification 3
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 3"].STRAT
    ).toBe(0);
    expect(visit2EdResults["PopulationCriteria1 - Stratification 3"].IPP).toBe(
      0
    );
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 3"].MSRPOPL
    ).toBe(0);
    expect(
      visit2EdResults["PopulationCriteria1 - Stratification 3"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit2EdResults["PopulationCriteria1 - Stratification 3"]
          .episode_results
      )
    ).toEqual([]);
    ...
    expect(
      getEpisodeResults(
        visit1exclResults["PopulationCriteria1"].episode_results
      )
    ).toEqual([25]);
    // Patient visit1exclResults Population Set 1 Stratification 1
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 1"].STRAT
    ).toBe(2);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 1"].IPP
    ).toBe(2);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 1"].MSRPOPL
    ).toBe(2);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 1"].MSRPOPLEX
    ).toBe(1);
    expect(
      getEpisodeResults(
        visit1exclResults["PopulationCriteria1 - Stratification 1"]
          .episode_results
      )
    ).toEqual([25]);
    // Patient visit1exclResults Population Set 1 Stratification 2
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 2"].STRAT
    ).toBe(0);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 2"].IPP
    ).toBe(0);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 2"].MSRPOPL
    ).toBe(0);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 2"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit1exclResults["PopulationCriteria1 - Stratification 2"]
          .episode_results
      )
    ).toEqual([]);
    // Patient visit1exclResults Population Set 1 Stratification 3
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 3"].STRAT
    ).toBe(0);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 3"].IPP
    ).toBe(0);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 3"].MSRPOPL
    ).toBe(0);
    expect(
      visit1exclResults["PopulationCriteria1 - Stratification 3"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit1exclResults["PopulationCriteria1 - Stratification 3"]
          .episode_results
      )
    ).toEqual([]);
    ...
    expect(
      getEpisodeResults(
        visit2exclResults["PopulationCriteria1"].episode_results
      )
    ).toEqual([]);
    // Patient visit2exclResults Population Set 1 Stratification 1
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 1"].STRAT
    ).toBe(2);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 1"].IPP
    ).toBe(2);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 1"].MSRPOPL
    ).toBe(2);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 1"].MSRPOPLEX
    ).toBe(2);
    expect(
      getEpisodeResults(
        visit2exclResults["PopulationCriteria1 - Stratification 1"]
          .episode_results
      )
    ).toEqual([]);
    // Patient visit2exclResults Population Set 1 Stratification 2
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 2"].STRAT
    ).toBe(0);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 2"].IPP
    ).toBe(0);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 2"].MSRPOPL
    ).toBe(0);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 2"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit2exclResults["PopulationCriteria1 - Stratification 2"]
          .episode_results
      )
    ).toEqual([]);
    // Patient visit2exclResults Population Set 1 Stratification 3
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 3"].STRAT
    ).toBe(0);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 3"].IPP
    ).toBe(0);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 3"].MSRPOPL
    ).toBe(0);
    expect(
      visit2exclResults["PopulationCriteria1 - Stratification 3"].MSRPOPLEX
    ).toBe(0);
    expect(
      getEpisodeResults(
        visit2exclResults["PopulationCriteria1 - Stratification 3"]
          .episode_results
      )
    ).toEqual([]);
    ...
    expect(
      visit1EdResults.PopulationCriteria1.statement_relevance
  ...
  xit("multiple observation measure correctly", () => {
    ...
    let statements = visit1EdResults.PopulationCriteria1.statement_results_by_statement()[
      "MedianTimefromEDArrivaltoEDDepartureforDischargedEDPatients"
    ];
    ...
    statements = visit1EdResults[
      "PopulationCriteria1 - Stratification 1"
    ].statement_results_by_statement()[
      "MedianTimefromEDArrivaltoEDDepartureforDischargedEDPatients"
    ];
    ...
    statements = visit1EdResults[
      "PopulationCriteria1 - Stratification 2"
    ].statement_results_by_statement()[
      "MedianTimefromEDArrivaltoEDDepartureforDischargedEDPatients"
    ];
    statementRelevanceOnly = {};
    ...
    statements = visit1EdResults[
      "PopulationCriteria1 - Stratification 3"
    ].statement_results_by_statement()[
      "MedianTimefromEDArrivaltoEDDepartureforDischargedEDPatients"
    ];
    statementRelevanceOnly = {};
  ...
  xit("single population EOC measure correctly", () => {
    ...
    const failIPPResults =
      calculationResults[Object.keys(calculationResults)[0]];
    const passNumerResults =
      calculationResults[Object.keys(calculationResults)[1]];
  ...
  it.skip("single population patient-based measure correctly", () => {
    ...
    const failHospiceNotPerformedDenex = getJSONFixture(
      "patients/CMS134v6/Fail_Hospice_Not_Performed_Denex.json"
    );
    ...
    const failHospiceNotPerformedDenexResults =
      calculationResults[Object.keys(calculationResults)[0]];
    const passNumerResults =
      calculationResults[Object.keys(calculationResults)[1]];

    expect(failHospiceNotPerformedDenexResults["PopulationCriteria1"].IPP).toBe(
      1
    );
    expect(
      failHospiceNotPerformedDenexResults["PopulationCriteria1"].DENOM
    ).toBe(1);
    expect(
      failHospiceNotPerformedDenexResults["PopulationCriteria1"].DENEX
    ).toBe(0);
    expect(
      failHospiceNotPerformedDenexResults["PopulationCriteria1"].NUMER
    ).toBe(0);
  ...
  it.skip("measure that calculates supplemental data elements correctly", () => {
    ...
    const passIppDenomNumer = getJSONFixture(
      "patients/CMS529v0/Pass_IPP-DENOM-NUMER.json"
    );
    const patients = [];
    ...
    const passIppDenomNumerResults =
      calculationResults[Object.keys(calculationResults)[0]];
  ...
  it.skip("multiple populations with multiple stratifications measure correctly", () => {
    ...
    const ippPopFail = getJSONFixture(
      "patients/CMS137v7/2YoungDependence&TX_IPPPopFail.json"
    );
    const denexPop18StratPass = getJSONFixture(
      "patients/CMS137v7/Dependency<60daysSB4_DENEXPop>18StratPass.json"
    );
    const pop1Pass = getJSONFixture(
      "patients/CMS137v7/Therapy<14DaysDx_NUMERPop1_13-18Pass.json"
    );
    const patients = [];
    ...
    const ippPopFailResults =
      calculationResults[Object.keys(calculationResults)[0]];
    const denexPop18StratPassResults =
      calculationResults[Object.keys(calculationResults)[1]];
    const pop1PassResults =
      calculationResults[Object.keys(calculationResults)[2]];
    ...
    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 1"].IPP
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 1"].DENOM
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 1"].NUMER
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 1"].DENEX
    ).toBe(0);

    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 2"].IPP
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 2"].DENOM
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 2"].NUMER
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria1 - Stratification 2"].DENEX
    ).toBe(0);
    ...
    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 1"].IPP
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 1"].DENOM
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 1"].NUMER
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 1"].DENEX
    ).toBe(0);

    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 2"].IPP
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 2"].DENOM
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 2"].NUMER
    ).toBe(0);
    expect(
      ippPopFailResults["PopulationCriteria2 - Stratification 2"].DENEX
    ).toBe(0);
    ...
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 1"].IPP
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 1"].DENOM
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 1"].NUMER
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 1"].DENEX
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 1"].STRAT
    ).toBe(0);

    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 2"].IPP
    ).toBe(1);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 2"].DENOM
    ).toBe(1);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 2"].NUMER
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 2"].DENEX
    ).toBe(1);
    expect(
      denexPop18StratPassResults["PopulationCriteria1 - Stratification 2"].STRAT
    ).toBe(1);
    ...
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 1"].IPP
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 1"].DENOM
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 1"].NUMER
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 1"].DENEX
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 1"].STRAT
    ).toBe(0);

    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 2"].IPP
    ).toBe(1);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 2"].DENOM
    ).toBe(1);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 2"].NUMER
    ).toBe(0);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 2"].DENEX
    ).toBe(1);
    expect(
      denexPop18StratPassResults["PopulationCriteria2 - Stratification 2"].STRAT
    ).toBe(1);
    ...
    expect(pop1PassResults["PopulationCriteria1 - Stratification 1"].IPP).toBe(
      1
    );
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 1"].DENOM
    ).toBe(1);
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 1"].NUMER
    ).toBe(1);
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 1"].DENEX
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 1"].STRAT
    ).toBe(1);

    expect(pop1PassResults["PopulationCriteria1 - Stratification 2"].IPP).toBe(
      0
    );
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 2"].DENOM
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 2"].NUMER
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 2"].DENEX
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 2"].STRAT
    ).toBe(0);
    ...
    expect(pop1PassResults["PopulationCriteria2 - Stratification 1"].IPP).toBe(
      1
    );
    expect(
      pop1PassResults["PopulationCriteria2 - Stratification 1"].DENOM
    ).toBe(1);
    expect(
      pop1PassResults["PopulationCriteria2 - Stratification 1"].NUMER
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria2 - Stratification 1"].DENEX
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria1 - Stratification 1"].STRAT
    ).toBe(1);

    expect(pop1PassResults["PopulationCriteria2 - Stratification 2"].IPP).toBe(
      0
    );
    expect(
      pop1PassResults["PopulationCriteria2 - Stratification 2"].DENOM
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria2 - Stratification 2"].NUMER
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria2 - Stratification 2"].DENEX
    ).toBe(0);
    expect(
      pop1PassResults["PopulationCriteria2 - Stratification 2"].STRAT
    ).toBe(0);
  });
  ...
  describe("opioid measure", () => {
    it.skip("results have correct indentation", () => {
      ...
      patients.push(
        getJSONFixture(
          "patients/CMS460v0/MethadoneLessThan90MME_NUMERFail.json"
        )
      );
      const options = { doPretty: true, requestDocument: true };
      const calculationResults = Calculator.calculate(
      ...
      const conversionFactorResult = Object.values(
        calculationResults[Object.keys(calculationResults)[1]]
      )[0];
      ...
      expect(
        resultsByStatement["PotentialOpioidOveruse"][
          "Periods With and Without 7 Day Gap With Cumulative Med Duration 90 days or Greater"
        ]["pretty"]
      ).toEqual(indentedResult);
      expect(
        conversionFactorResultsByStatement["PotentialOpioidOveruse"][
          "Prescriptions with MME"
        ]["pretty"]
      ).toContain("conversionFactor: 4,");
    });
  });
  ...
  describe("results include clause results", () => {
    it.skip("only if requested with document", () => {
      ...
      const calculationResultsNoClauses = Calculator.calculate(
        measure,
        patients,
        valueSets,
        {
      ...
      const resultNoClauses = Object.values(
        calculationResultsNoClauses[Object.keys(calculationResultsNoClauses)[0]]
      )[0];

      const calculationResultsWithClauses = Calculator.calculate(
        measure,
        patients,
        valueSets,
        {
      ...
      const resultWithClauses = Object.values(
        calculationResultsWithClauses[
          Object.keys(calculationResultsWithClauses)[0]
        ]
      )[0];
  ...
  describe("patient characteristics", () => {
    it.skip("calculate and display correctly", () => {
      ...
      const patients = [];
      patients.push(
        getJSONFixture("patients/CMS123v7/Test_PatientCharacteristic.json")
      );
      ...
      expect(
        resultsByStatement["DiabetesFootExam"]["SDE Ethnicity"]["pretty"]
      ).toEqual("[PatientCharacteristicEthnicity\nCODE: CDCREC 2186-5]");
      const expectedPayer =
        "[Patient Characteristic Payer: Payer\nSTART: 07/25/2012 8:00 AM\nSTOP: 07/25/2012 8:15 AM\nCODE: Source of Payment Typology 1]";
      expect(
        resultsByStatement["DiabetesFootExam"]["SDE Payer"]["pretty"]
      ).toEqual(expectedPayer);
      expect(
        resultsByStatement["DiabetesFootExam"]["SDE Race"]["pretty"]
      ).toEqual("[PatientCharacteristicRace\nCODE: CDCREC 1002-5]");
      expect(
        resultsByStatement["DiabetesFootExam"]["SDE Sex"]["pretty"]
      ).toEqual("[PatientCharacteristicSex\nCODE: AdministrativeGender M]");
    });
  });

  describe("Id objects", () => {
    it.skip("properly calculate and print", () => {
      ...
      const patients = [
        getJSONFixture(
          "patients/CMS108v7/INR4no_decimal_DayOfAnes_NUMERPass.json"
        ),
      ];
      const options = { doPretty: true, requestDocument: true };
      const calculationResults = Calculator.calculate(
      ...
      const sampleResult =
        resultsByStatement["VenousThromboembolismProphylaxis"][
  ...
  describe("Non-Versioned Value-Sets", () => {
    xit("Calculates correctly", () => {
      ...
      const patients = [
        getJSONFixture("patients/CMS105v7/StatinAtDC_NUMERPass.json"),
      ];
      const options = { doPretty: true, requestDocument: true };
      const calculationResults = Calculator.calculate(
        measure,
  ...
  describe("Using cqm-models passed in", () => {
    it.skip("Doesn't pollute Measure model to calculate measure with stratification", () => {
      const valueSets = getJSONFixture("cqm_measures/CMS160v7/value_sets.json");
      const measure = getJSONFixture("cqm_measures/CMS160v7/CMS160v7.json");
      const numPass = getJSONFixture(
        "patients/CMS160v7/PHQ9EBEDec_PerDzDxSAEDec_NUM1Pass.json"
