...
module.exports = class Executor {
  ...
  */
   ...
   */
  execute(patientSource, valueSetSource, measureIds, handler, options) {
    const measures = this.getMeasuresById(measureIds, options);
    const executor = this;
    handler.start();

    valueSetSource.findValueSetsByOidForMeasures(measures, (self) => {
      let patient = patientSource.nextPatient();
      while (patient != null) {
        executor.executeSinglePatient(patient, self.valueSetsByOid, measures, handler, options);
  ...
  static executeSinglePatient(patient, valueSetsByOid, measures, handler, options) {
    measures.forEach((mes) => {
      const result = Result();

      result.calculate(patient, mes, valueSetsByOid, options.effectiveDate, options.correlationId); // TODO: What are the two options for?
