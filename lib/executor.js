...
module.exports = class Executor {
  ...
  */
  ...
  execute(patient_source, measure_ids, handler, options) {
    ...
    handler.start();
    let measures = this.generate_measures(measure_ids, options);
    let patient = null;
    while( patient = patient_source.next() ) {
      measures.forEach( mes => {
        let result = new Result();
        result.calculate(patient, measure, options.effective_date, options.correlation_id);
        handler.handleResult(patient, measure, result);
      });
    }
