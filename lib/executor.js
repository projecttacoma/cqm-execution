...
module.exports = class Executor {
  ...
  */
   ...
   */
  async execute(patientIds, measureIds, options) {
    ...
    if (!options.handlerType || options.handlerType === 'CypressMongo') {
    ...
    await this.valueSetSource.findValueSetsByMongoidForMeasures(measures, (self) => {
      executor.executeForEachMeasure(self.valueSetsByMongoid, measures, handler);
    });
    return handler.finish();
  }

  executeForEachMeasure(measures, valueSetsByMongoid, handler) {
    measures.forEach((mes) => {
      ...
      const resultsByPid = this.cqlCalculator.calculate(
        mes,
        this.patientSource, valueSetsByMongoid
      );
