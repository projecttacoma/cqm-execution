...
module.exports = class CQLCalculator {
  ...
  cacheKey(measure, patient) { return `${this.calculationKey(measure)}/${patient.cid}`; }
   ...
   */
  calculate(measure, patientSource, valueSetsByOid) {
    // TODO: Replace all functionality dealing with the single patient with functionality
    // dealing with the patientSource instead
    const patient = patientSource.currentPatient();
    ...
    try {
      ...
      const start =
        this.getConvertedTime(measure.get('measure_period').low.value);
      const end =
        this.getConvertedTime(measure.get('measure_period').high.value);
      ...
      const measureValueSets =
        this.valueSetsForCodeService(
          measure.get('value_set_oid_version_objects'),
      ...
      const [populationResults, episodeResults] =
        Array.from(this.createPopulationValues(measure, results, patient, observationDefs));
      ...
      if (populationResults != null) {
        ...
        if (episodeResults != null) {
          ...
          // for EACH episode.
          result.set({ episodeResults });
        }
        result.embedded.patient_id = patient.id; // Add patient_id to result in order to delete patient from population_calculation_view
      }
    } catch (error) {
      Error(error);
    }
   ...
   */
  createPopulationValues(measure, results, patient, observationDefs) {
    ...
    if (!measure.get('episode_of_care')) {
      measure.get('populations').forEach((population) => {
        populationResults =
          this.createPatientPopulationValues(
            ...
            population, results, patient, observationDefs,
          );
      });
      ...
      Measure.allPopulationCodes.forEach((popCode) => {
        if (popCode === 'OBSERV') {
  ...
  handlePopulationValues(populationResults) {
    ...
    if ((populationResultsHandled.STRAT != null) && this.isValueZero('STRAT', populationResults)) { // Set all values to 0
    ...
    } else if (this.isValueZero('DENOM', populationResults) ||
    ...
    } else if ((populationResults.DENEX != null) && !this.isValueZero('DENEX', populationResults) &&
  ...
  static createPatientPopulationValues(
    ...
    results, patient, observationDefs,
  ) {
    const populationResultsHandled = populationResults;
    ...
    Measure.allPopulationCodes.forEach((popCode) => {
      ...
      if (cqlMap[popCode]) {
        ...
        const popIndex = population.getPopIndexFromPopName(popCode);
        ...
        if (population.get(popCode) != null) {
          // Grab CQL result value and adjust for Bonnie
          value = results.patientResults[patient.id][cqlPopulation];
        ...
        observationDefs.forEach((obDef) => {
          ...
          const obsResults =
            (results.patientResults != null && results.patientResults[patient.id] != null) ?
              results.patientResults[patient.id][obDef] : undefined;
    ...
    popCodesInPopulation.forEach((popCode) => {
      ...
      if (cqlMap[popCode]) {
        ...
        if (population.get(popCode) != null) {
          ...
          values = results.patientResults[patient.id][cqlPopulation];
  ...
  static valueSetsForCodeService(valueSetOidVersionObjects, hqmfSetId) {
    // Cache this refactoring so it only happens once per user rather than once per measure population
    if (!this.valueSetsByOidCached[hqmfSetId]) {
      const valueSets = {};
      valueSetOidVersionObjects.forEach((valueSet) => {
        const specificValueSet = this.valueSetsByOid[valueSet.oid][valueSet.version];
      ...
      this.valueSetsByOidCached[hqmfSetId] = valueSets;
    }
    return this.valueSetsByOidCached[hqmfSetId];
  }
