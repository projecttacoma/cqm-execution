...
module.exports = class Calculator {
  ...
  static async calculate(
    ...
    const qdmPatients = await patients.map((patient) => {
    ...
    //returns a promise that resolves to an object with patientResults and localIdPatientResultsMap
    const resultsRaw = await Calculator.executeEngine(
      allElm,
      patientSource,
      measureValueSets,
        ...
        if (populationResults) {
          ...
          if (episodeResults != null) {
            ...
            if (Object.keys(episodeResults).length > 0) {
               ...
               */
              populationRelevance = ResultsHelpers.populationRelevanceForAllEpisodes(episodeResults, measure.measure_scoring);
            } else {
              // Use the patient based relevance if there are no episodes. This will properly set IPP or STRAT to true.
              populationRelevance = ResultsHelpers.buildPopulationRelevanceMap(populationResults, measure.measure_scoring);
            }
          } else {
   ...
   */
  static async executeEngine(elm, patientSource, valueSets, libraryName, version, executionDateTime, parameters = {}) {
    let lib;
    let rep;
    ...
    return await executor.exec(patientSource, executionDateTime);    
  }
};