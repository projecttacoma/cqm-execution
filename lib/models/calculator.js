...
module.exports = class Calculator {
   ...
   */
  static calculate(measure, patients, valueSetsByOid, requestOptions = {}) {
    //define defaults for options
    const options = {
      includeClauseResults: false,
      doPretty: false
    }
    Object.entries(requestOptions).forEach(([key, value]) => options[key] = value);
    ...
    // Set a timezone offset from options if it was included
    let timezoneOffset = 0;
    if (options && options.timezone_offset) {
      timezoneOffset = options.timezone_offset;
    }

    // Create the execution DateTime that we pass into the engine
    const executionDateTime = cql.DateTime.fromDate(executionDateOption, timezoneOffset);
    ...
    Object.keys(resultsRaw.patientResults).forEach((patientId) => {
      ...
      measure.populations.forEach((population, populationIndex) => {
        ...
        if (populationResults) {
          ...
          result.set(ResultsHelpers.buildStatementAndClauseResults(measure, resultsRaw.localIdPatientResultsMap[patientId], statementRelevance, !!options.doPretty, !!options.includeClauseResults));
