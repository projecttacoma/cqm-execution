...
const ValueSet = models.ValueSet;
...
module.exports = class Calculator {
  ...
  static calculate(
    ...
    const start = CalculatorHelpers.parseTimeStringAsUTC(
      measure.measure_period.start
    );
    const end = CalculatorHelpers.parseTimeStringAsUTC(
      measure.measure_period.end
    );
    ...
    const executionDateOption = new Date();

    // Create the execution DateTime that we pass into the engine
    const executionDateTime = cql.DateTime.fromJSDate(executionDateOption, 0);
    ...
    const measureValueSets = CalculatorHelpers.valueSetsForCodeService(
      valueSets.map((valueSet) => {
        ...
        return ValueSet.parse(valueSet);
      })
    );
  ...
  static convertPatientToBundle(cqmPatient) {
    ...
    if (cqmPatient.fhir_patient) {
      const { identifier, meta } = Calculator.staticPatientElements(cqmPatient);
      ...
      patientEntry.resource.identifier = [identifier];
      patientEntry.resource.meta = meta;
  ...
  static staticPatientElements(cqmPatient) {
    const identifier = new models.Identifier();
    identifier.type = new models.CodeableConcept();
    identifier.type.coding = [new models.Coding()];
    identifier.type.coding[0].system = models.PrimitiveUri.parsePrimitive("http://terminology.hl7.org/CodeSystem/v2-0203");
    identifier.type.coding[0].code = models.PrimitiveCode.parsePrimitive("MR");
    identifier.system = models.PrimitiveUri.parsePrimitive("https://bonnie-fhir.healthit.gov/");
    identifier.value = models.PrimitiveString.parsePrimitive(cqmPatient.id);

    const meta = new models.Meta();
    meta.profile = [models.PrimitiveUri.parsePrimitive("http://hl7.org/fhir/us/qicore/StructureDefinition/qicore-patient")];
