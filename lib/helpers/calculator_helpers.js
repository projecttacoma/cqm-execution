...
module.exports = class CalculatorHelpers {
   ...
   * TODO: This funciton will be irrelevant once we start using the new measure model
   * @private
  ...
  static getPopulationFromIndex(definedPops, populationIndex, population, popCode) {
    ...
    if (population.population_index != null && popCode !== 'STRAT') {
      ...
      const strat_name = population['STRAT']
      const stratification_index = this.getStratIndexFromStratName(strat_name)
      cqlPopulation = definedPops[stratification_index];
    }
