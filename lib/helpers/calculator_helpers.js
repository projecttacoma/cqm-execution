...
module.exports = class CalculatorHelpers {
  ...
  static createPopulationValues(measure, populationSet, patientResults, observationDefs) {
    ...
    if (measure.calculation_method !== 'EPISODE_OF_CARE') {
      ...
      populationResults = this.handlePopulationValues(populationResults, measure.measure_scoring, populationSet);
    } else {
   ...
   */
  static handlePopulationValues(populationResults, measureScoring, populationSet) {
    ...
    if (measureScoring === 'RATIO') {
      var denom_ipp = 'IPP'
      var numer_ipp = 'IPP'
      if ('IPP_1' in populationResults) {
        if (populationSet.populations['NUMER'].subject_id == populationSet.populations['IPP_1'].hqmf_id){
          numer_ipp = 'IPP_1'
        }
        if (populationSet.populations['DENOM'].subject_id == populationSet.populations['IPP_1'].hqmf_id){
          denom_ipp = 'IPP_1'
        }
      }
      ...
      if (this.isValueZero(denom_ipp, populationResults)) {
        populationResultsHandled.DENOM = 0;
      ...
      if (this.isValueZero('DENOM', populationResults)) {
        if ('DENEX' in populationResults) {
          populationResultsHandled.DENEX = 0;
        }
        if ('observation_values' in populationResults) {
      ...
      if (this.isValueZero(numer_ipp, populationResults)) {
   ...
   */
  static createEpisodePopulationValues(populationSet, patientResults, observationDefs, measureScoring) {
    ...
    Object.keys(episodeResults).forEach((episodeId) => {
      ...
      // Correct any inconsistencies. ex. In DENEX but also in NUMER using same function used for patients.
      episodeResults[episodeId] = this.handlePopulationValues(episodeResult, measureScoring, populationSet);
    });
