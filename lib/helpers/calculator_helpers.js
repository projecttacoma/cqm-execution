  ...
  static createPatientPopulationValues(
    ...
    if ((observationDefs != null ? observationDefs.length : undefined) > 0) {
      ...
      observationDefs.forEach((obDef) => {
        ...
        const obsResults =
          patientResults != null ? patientResults[obDef] : undefined;
        ...
        if (obsResults) {
          ...
          obsList.forEach((obsResult) => {
            ...
            if (!obsResult.episode) {
              console.log("Warning: Observation's episode is undefined. Perhaps not an EPOSIDE_OF_CARE measure.");
              return;
            }
    ...
    valueSetsArray.forEach((valueSet) => {
        ...
        valueSet.compose.include.forEach((include) => {
          ...
          include.concept.forEach((concept) => {
            if (!valueSets[valueSet.id]) {
              valueSets[valueSet.id] = {};
            }
            if (!valueSets[valueSet.id][version]) {
              valueSets[valueSet.id][version] = [];
            }
            valueSets[valueSet.id][version].push({
              code: concept.code,
