...
module.exports = class CalculatorHelpers {
  ...
  static createEpisodePopulationValues(
    ...
    if (populationSet.populations) {
      for (const popCode of populationSet.populations.codes) {
        let newEpisode;
          ...
          if (Array.isArray(dataElements)) {
            dataElements.forEach((dataElement) => {
              if (dataElement.id != null) {
                ...
                if (episodeResults[dataElement.id.value]) {
                  episodeResults[dataElement.id.value][popCode] = 1;
                  // else create a new episode using the list of all popcodes for the population
                } else {
                  newEpisode = {};
                  for (const pc of populationSet.populations.codes) {
                    newEpisode[pc] = 0;
                  }

                  newEpisode[popCode] = 1;
                  episodeResults[dataElement.id.value] = newEpisode;
                }
              }
            });
          }
  ...
  static valueSetsForCodeService(valueSetsArray) {
    ...
    valueSetsArray.forEach((valueSet) => {
      ...
      valueSet.compose.include.forEach((include) => {
        ...
        // TODO: VS version is not supported at the moment
        const version = "";
  ...
  static systemUriByName(systemName) {
    ...
    map["LOINC"] = "http://loinc.org";
    map["SNOMEDCT"] = "http://snomed.info/sct";
    map["SNOMEDCT:2017-09"] = "http://snomed.info/sct";
    map["CPT"] = "http://www.ama-assn.org/go/cpt";
    map["SOP"] = "http://www.nlm.nih.gov/research/umls/sop";
    map["ICD9CM"] = "http://hl7.org/fhir/sid/icd-9-cm";
    map["ICD10PCS"] = "http://www.icd10data.com/icd10pcs";
    map["ICD10CM"] = "http://hl7.org/fhir/sid/icd-10-cm";
