...
module.exports = class ResultsHelpers {
  ...
  /**
    ...
    * @param {PopulationSet} populationSet - The populationSet we wish to get statement relevance for.
    * @return {object} Statement relevance map for the population set.
    */

  static getStatementRelevanceForPopulationSet(cqmMeasure, populationSet) {
    // create a population relevance map where every population is true.
    const stratificationPopulations = CalculatorHelpers.getStratificationsAsPopulationSets(cqmMeasure);
    // TODO: port this from the thorax measure into separate function and call that from bonnie instead
    const popSetsAndStrats = cqmMeasure.population_sets.concat(stratificationPopulations)
      .map((popSet) => {
        if (typeof popSet.toObject === 'function') {
          return popSet.toObject();
        }
        return popSet;
      });
    // EndTODO
    ...
    // OBSERVs are stored as 'values'
    populationRelevance.values = true;
    for (const populationCode of popSetsAndStrats) {
      populationRelevance[populationCode] = true;
    }

    // builds and returns this statement relevance map.
    return this.buildStatementRelevanceMap(populationRelevance, cqmMeasure, populationSet);
  }
};
