...
module.exports = class ResultsHelpers {
  ...
  static markStatementRelevant(cqlStatementDependencies, statementRelevance, libraryName, statementName, relevant) {
    ...
    if (
      ...
      const library = cqlStatementDependencies.find(lib => {return lib.library_name == libraryName})
      const statement = library.statement_dependencies.find(statement => {return statement.statement_name == statementName})
    ...
    for (const library of measure.cql_libraries) {
      ...
      for (const statement of statements) {
        ...
        if ((!measure.calculate_sdes && isSDE) || statementRelevance[libraryName][statementName] === 'NA') {
          ...
          if (doPretty) {
            if (MeasureHelpers.isStatementFunction(measure, libraryName, statementName)) {
  ...
  static findResultForStatementClause(library, statement, rawClauseResults) {
    let elm_statement  = library.elm.library.statements.def.find(def=>{return def.name == statement.statement_name});
    const libraryName = library.library_name;
    return rawClauseResults[libraryName] != null ? rawClauseResults[libraryName][elm_statement.localId] : undefined;
  }
