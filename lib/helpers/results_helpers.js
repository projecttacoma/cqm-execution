...
module.exports = class ResultsHelpers {
  ...
  static buildStatementRelevanceMap(populationRelevance, measure, populationSet, populationIndex) {
    ...
    for (const population in populationRelevance) {
      ...
      if (population === 'values') {
        ...
        if (population === 'STRAT') {
          relevantStatement = measure.populations_cql_map[population][populationSet.stratification_index];
  ...
  static buildStatementAndClauseResults(measure, rawClauseResults, statementRelevance, doPretty, includeClauseResults) {
    ...
    let response = { statement_results: statementResults };
    response.clause_results = includeClauseResults ? clauseResults : "These will be included if you set includeClauseResults to true in the request options."
    return response;
  }
  ...
  static prettyResult(result, indentLevel, keyIndent) {
    ...
    if (result instanceof cql.DateTime) {
    ...
    } else if (result && result._type && result._type.includes('QDM::')) {
      ...
      if (result.relevantPeriod) {
        if (result.relevantPeriod.low) {
          startTime = `START: ${moment.utc(result.relevantPeriod.low.toString()).format('MM/DD/YYYY h:mm A')}\n`;
        }
        if (result.relevantPeriod.high) {
          endTime = `STOP: ${moment.utc(result.relevantPeriod.high.toString()).format('MM/DD/YYYY h:mm A')}\n`;
        }
      } else if (result.prevalencePeriod) {
        if (result.prevalencePeriod.low) {
          startTime = `START: ${moment.utc(result.prevalencePeriod.low.toString()).format('MM/DD/YYYY h:mm A')}\n`;
        }
        if (result.prevalencePeriod.low) {
          endTime = `STOP: ${moment.utc(result.prevalencePeriod.high.toString()).format('MM/DD/YYYY h:mm A')}\n`;
        }
      } else if (result.authorDatetime) {
        // TODO: this will need to be updated to AUTHORED once bonnie frontend
        // updates its pretty printer to do so.
        startTime = `START: ${moment.utc(result.authorDatetime.toString()).format('MM/DD/YYYY h:mm A')}\n`;
      }

      // If endTime is the infinity dateTime, clear it out because we do not want to export it
      if (endTime === 'STOP: 12/31/9999 11:59 PM\n') {
        endTime = '';
      }
      ...
      // Add indentation
      const returnString = `${description}${startTime}${endTime}${codeDisplay}`;
