...
module.exports = class ResultsHelpers {
  ...
  static buildStatementRelevanceMap(populationRelevance, measure, populationSet) {
    ...
    if (measure.calculate_sdes && populationSet && populationSet.supplemental_data_elements) {
      for (const statement of Array.from(populationSet.supplemental_data_elements)) {
        ...
        this.markStatementRelevant(measure.cql_libraries, statementRelevance, measure.main_cql_library, statement.statement_name, "TRUE");
      }
    }
    ...
    for (const population in populationRelevance) {
      ...
      if (population === "observation_values") {
        ...
        this.markStatementRelevant(measure.cql_libraries, statementRelevance, measure.main_cql_library, relevantStatement, relevance);
      }
    }
  ...
  static markStatementRelevant(cqlLibraries, statementRelevance, libraryName, statementName, relevant) {
    ...
    if (statementRelevance[libraryName][statementName] === "NA" || statementRelevance[libraryName][statementName] === "FALSE") {
      ...
      if (!library.statement_dependencies) {
        return [];
      }
      ...
      return statement.statement_references.map((dependentStatement) =>
        this.markStatementRelevant(cqlLibraries, statementRelevance, dependentStatement.library_name, dependentStatement.statement_name, relevant)
      );
    }
    return [];
  }
  ...
  static buildStatementAndClauseResults(measure, rawClauseResults, statementRelevance, doPretty, includeClauseResults) {
    ...
    for (const library of measure.cql_libraries) {
      ...
      for (const statement of statements) {
        ...
        if (rawStatementResult && Array.isArray(rawStatementResult)) {
        ...
        if ((!measure.calculate_sdes && isSDE) || relevance === "NA") {
          ...
          if (Array.isArray(rawStatementResult) && rawStatementResult.length === 0) {
          ...
          for (const localId in localIds) {
            ...
            let rawClauseResult =
              rawClauseResults[library_name] != null
                ...
                : undefined;
            if (rawClauseResult && Array.isArray(rawClauseResult)) {
  ...
  static prettyResult(result, indentLevel, keyIndent) {
    ...
    if (result instanceof cql.DateTime) {
      ...
      const codeDisplay =
        result.dataElementCodes && result.dataElementCodes[0] ? `CODE: ${nameOidHash[system] || system} ${result.dataElementCodes[0].code}` : "";
      // Add indentation
  ...
  static setFinalResults(params) {
    ...
    if (params.clause.isUnsupported != null) {
      ...
      finalResult = "NA";
    } else if (params.statementRelevance[params.library_name][params.statement_name] === "FALSE" || params.rawClauseResults[params.library_name] == null) {
      finalResult = "UNHIT";
  ...
  static buildPopulationRelevanceMap(result) {
    ...
    const resultShown = {};
    _.each(Object.keys(result), (population) => {
      resultShown[population] = true;
    });
