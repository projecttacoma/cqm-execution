    ...
    for (const library of measure.cql_libraries) {
      ...
      for (const statement of statements) {
        ...
        if (includeClauseResults) {
          let localIds;
          if (this.localIdsCache == null) {
            this.localIdsCache = {};
          }
          if (this.localIdsCache[library_name] == null) {
            this.localIdsCache[library_name] = {};
          }
          if (this.localIdsCache[library_name][statement_name] !== null) {
            console.log("cache hit");
            localIds = this.localIdsCache[library_name][statement_name];
          } else {
            // create clause results for all localIds in this statement and add
            // to the cache
            console.log("cache miss");
            localIds = MeasureHelpers.findAllLocalIdsInStatementByName(library.elm, statement_name);
            this.localIdsCache[library_name][statement_name] = localIds;
          }
