...
module.exports = class MeasureHelpers {
  ...
  static findAllLocalIdsInStatementByName(libraryElm, statementName) {
    ...
    const localIds = this.findAllLocalIdsInStatement(statement, libraryName, statement.annotation, {}, aliasMap, emptyResultClauses, null);
   ...
   */
  static findAllLocalIdsInStatement(statement, libraryName, annotation, localIds, aliasMap, emptyResultClauses, parentNode) {
    ...
    for (const k in statement) {
      ...
      if (k === "return") {
        ...
        for (const aLet of Array.from(v)) {
          ...
          this.findAllLocalIdsInStatement(aLet.expression, libraryName, annotation, localIds, aliasMap, emptyResultClauses, statement);
        }
        ...
        const libraryClauseLocalId = this.findLocalIdForLibraryRef(annotation, statement.localId, statement.libraryName);
  ...
  static findLocalIdForLibraryRef(annotation, refLocalId, libraryName) {
    ...
    if (Array.isArray(annotation)) {
      ...
      if (annotation.r != null && annotation.r === refLocalId) {
        ...
        if (this.__guard__(annotation.s[0].s != null ? annotation.s[0].s[0].value : undefined, (x) => x[0]) === libraryName) {
