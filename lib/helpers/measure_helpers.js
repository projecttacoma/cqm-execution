...
module.exports = class MeasureHelpers {
  ...
  static findAllLocalIdsInStatementByName(libraryElm, statementName) {
    ...
    const statement = libraryElm.library.statements.def.find(statement => {return statement.name == statementName});
  ...
  static isStatementFunction(library, statementName) {
    ...
    const statement = library.elm.library.statements.def.find(def => {return def.name == statementName})
    if (statement != null) {
  ...
  static isSupplementalDataElementStatement(populationSets, statementName) {
    for (var populationSet of populationSets){
      if(populationSet.supplemental_data_elements){
        const sde_statement = populationSet.supplemental_data_elements.find(sde => {return sde.statement_name == statementName});
        if (sde_statement != undefined){
          return true;
        }
      }
    }
